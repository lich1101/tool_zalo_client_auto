...
namespace webview_cef {
	...
	void WebviewPlugin::initCallback() {
		if (!m_init)
		{
			...
			m_handler->onBeforePopupEvent = [=, this](int browserId, std::string url)
			{
				if (m_invokeFunc)
				{
					WValue* bId = webview_value_new_int(browserId);
					...
					webview_value_set_string(retMap, "browserId", bId);
					webview_value_set_string(retMap, "url", wUrl);
					m_invokeFunc("popupRequest", retMap);
					webview_value_unref(bId);
					webview_value_unref(wUrl);
					webview_value_unref(retMap);
				}
	...
	void WebviewPlugin::uninitCallback(){
		...
		m_handler->onBeforePopupEvent = nullptr;
    ...
    void WebviewPlugin::HandleMethodCall(std::string name, WValue* values, std::function<void(int ,WValue*)> result) {
		if (name.compare("init") == 0){
			if(!isCefInitialized){
				if (values != nullptr &&
				    webview_value_get_type(values) == Webview_Value_Type_List) {
					if (auto uaValue = webview_value_get_list_value(values, 0)) {
						const auto rawUa = webview_value_get_string(uaValue);
						if (rawUa != nullptr) {
							userAgent = CefString(rawUa);
						}
					}
					if (auto rootValue = webview_value_get_list_value(values, 1)) {
						const auto rawRoot = webview_value_get_string(rootValue);
						if (rawRoot != nullptr) {
							rootCachePath = CefString(rawRoot);
						}
					}
			...
			if (values != nullptr &&
			    webview_value_get_type(values) == Webview_Value_Type_List) {
				if (auto urlValue = webview_value_get_list_value(values, 0)) {
					const auto rawUrl = webview_value_get_string(urlValue);
					if (rawUrl != nullptr) {
						url = rawUrl;
					}
				}
				if (auto cachePathValue = webview_value_get_list_value(values, 1)) {
					const auto rawCachePath =
					    webview_value_get_string(cachePathValue);
					if (rawCachePath != nullptr) {
						cachePath = rawCachePath;
					}
				}
			} else if (values != nullptr) {
				const auto rawUrl = webview_value_get_string(values);
				if (rawUrl != nullptr) {
					url = rawUrl;
				}
			}
			m_handler->createBrowser(url, cachePath, [=, this](int browserId) {
	...
	void startCEF()
	{
		...
		// With the Chrome runtime each browser's per-profile cache_path must be a
		// child of root_cache_path. Set it to the app-provided root so isolated
		// profiles (root_cache_path/profiles/<uuid>) can be created.
