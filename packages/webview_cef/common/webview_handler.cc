...
void WebviewHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser) {
    ...
    if (!browser->IsPopup()) {
        ...
        // Deliver the id for an async (per-profile) CreateBrowser back to the
        // platform layer so it can wire up the texture. FIFO: createBrowser
        // pushed one callback per pending async creation.
        if (!pending_create_callbacks_.empty()) {
            auto callback = pending_create_callbacks_.front();
            pending_create_callbacks_.pop_front();
...
bool WebviewHandler::OnBeforePopup(CefRefPtr<CefBrowser> browser,
    ...
    // Block CEF's own popup (it would be a windowless browser with nowhere to
    // render) and hand the URL to the host, which opens it in a dedicated in-app
    // popup window backed by the same profile. Posting as a UI-thread task keeps
    // us out of the popup-decision call stack.
    const std::string url = target_url.ToString();
    if (!url.empty() && onBeforePopupEvent) {
        // Forwards to Dart over the platform channel (not a CEF navigation), so
        // it is safe to call directly from within OnBeforePopup.
        onBeforePopupEvent(browser->GetIdentifier(), url);
    }
    return true;
}
                                   ...
                                   std::function<void(int)> callback)
{
    ...
    if(!CefCurrentlyOn(TID_UI)) {
		CefPostTask(
            TID_UI,
            base::BindOnce(&WebviewHandler::createBrowser, this, url, cachePath,
                           callback));
		return;
	}
#endif
...
#endif
    if (!cachePath.empty()) {
        // Per-profile (isolated) browser. Under the CEF Chrome runtime a fresh
        // request-context profile loads ASYNCHRONOUSLY, so CreateBrowserSync
        // returns null before the Profile is ready. Use the async CreateBrowser
        // and deliver the browser id from OnAfterCreated (FIFO) once the profile
        // is up. This is what gives each account its own cookies/storage.
        ...
        pending_create_callbacks_.push_back(callback);
        const bool posted = CefBrowserHost::CreateBrowser(
            window_info, this, url, browser_settings, nullptr, request_context);
        if (!posted) {
            pending_create_callbacks_.pop_back();
            fprintf(stderr,
                    "[webview_cef] CreateBrowser(async) failed to post "
                    "(cachePath=%s)\n", cachePath.c_str());
            callback(-1);
        }
        return;
    ...
    // Global (shared) context — synchronous path.
    CefRefPtr<CefBrowser> browser = CefBrowserHost::CreateBrowserSync(
        window_info, this, url, browser_settings, nullptr, nullptr);
    if (!browser) {
        fprintf(stderr,
                "[webview_cef] CreateBrowserSync returned null (url=%s)\n",
                url.c_str());
        callback(-1);
        return;
    }
