...
public:
    ...
    // Fired when a page requests a popup / new tab (window.open, target=_blank).
    // The host opens the URL in a separate in-app popup window sharing the same
    // profile, instead of CEF creating an (unsupported, windowless) popup.
    std::function<void(int browserId, std::string url)> onBeforePopupEvent;
...
private:
    ...
    // Pending createBrowser callbacks for the async (per-profile request
    // context) path. The Chrome runtime loads a fresh profile asynchronously,
    // so CreateBrowserSync returns null; we use async CreateBrowser instead and
    // deliver the browser id from OnAfterCreated in FIFO order. Only accessed on
    // the CEF UI thread.
    std::list<std::function<void(int)>> pending_create_callbacks_;
