...
# Codesign rejects downloaded bundles that retain provenance/resource-fork style
# extended attributes after extraction/copy. Clear them once in the prepared
# source bundle so future app builds inherit a clean framework tree.
xattr -cr "${DEST}" || true
