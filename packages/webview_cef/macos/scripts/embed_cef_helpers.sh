...
# The embedded "Chromium Embedded Framework.framework" (and other copied-in
# files) can carry com.apple.FinderInfo / resource-fork xattrs — e.g. when the
# host project lives in an iCloud-synced folder that re-stamps them. Those break
# the host app's final code signing with:
#   "resource fork, Finder information, or similar detritus not allowed".
# Strip them from the whole host bundle here, after everything is embedded and
# right before Xcode's implicit CodeSign of the app.
