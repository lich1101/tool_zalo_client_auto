...
class WebViewController extends ValueNotifier<bool> {
  ...
  Future<void> initialize(String url, {String? cachePath}) async {
    if (_isDisposed) {
    ...
    try {
      ...
      final dynamic createArgs =
          cachePath != null && cachePath.isNotEmpty ? [url, cachePath] : url;
      List args = await _pluginChannel.invokeMethod('create', createArgs);
...
class WebViewState extends State<WebView> with WebeViewTextInput {
  ...
  void initState() {
    ...
    // Tooltips disabled by host: a CEF OSR build has no native browser tooltip
    // surface, so anything we render here is purely decorative — and on heavy
    // pages (Zalo Web has many nested `title` attributes) it just stacks up
    // overlays in the user's face. Drop the event on the floor.
