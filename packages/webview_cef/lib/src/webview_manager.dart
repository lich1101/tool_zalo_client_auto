  ...
  Future<void> initialize({String? userAgent, String? rootCachePath}) async {
    ...
    try {
      if (rootCachePath != null && rootCachePath.isNotEmpty) {
        // Pass [userAgent, rootCachePath] so the native side can set
        // CefSettings.root_cache_path (required by the Chrome runtime for
        // per-profile cache_path isolation).
        await pluginChannel
            .invokeMethod('init', <String>[userAgent ?? '', rootCachePath]);
      } else if (userAgent != null && userAgent.isNotEmpty) {
